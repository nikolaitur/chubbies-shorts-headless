...
import { json, redirect, type LoaderArgs, type MetaFunction } from '@shopify/remix-oxygen'
...
export async function loader({ request, context, params }: LoaderArgs) {
  ...
  const data = await context.storefront.query<{ node: Order }>(CUSTOMER_ORDER_QUERY, {
    variables: { orderId },
  })
...
export default function OrderRoute() {
  ...
  return (
    <div>
      <Link to="/account">Return to Account Overview</Link>
      <div className="w-full p-6 sm:grid-cols-1 md:p-8 lg:p-12 lg:py-6">
        <div>
          <h3>Order No. {order.name}</h3>
          <p className="mt-2">Placed on {new Date(order.processedAt!).toDateString()}</p>
                      ...
                      <div className="flex gap-6">
                        ...
                        <dl className="grid">
                          ...
                          <dd className="truncate lg:hidden">
                            ...
                            <div className="mt-1">{lineItem.variant!.title}</div>
                          </dd>
